---
title: "Recovery"
date: 2025-11-13 18:00:00 +0000
categories: [Writeups, Forensics, Securinets Quals CTF 2025]
tags: [forensics, writeup]
description: Securinets Quals CTF 2025 - Recovery Writeup
---

## Description

Challenge had no description, expect for a warning:

> *PS: Don't run the malware on your machine, it might be dangerous.*
{: .prompt-danger }
...
![](assets/img/Recovery/base32.png)
...
![](assets/img/Recovery/exe.png)

> A DOS header has the following signature: the first two bytes are `4D 5A ("MZ")`, the initials of Mark Zbikowski, which identifies the file as a DOS executable.
{: .prompt-tip }

This confirms that we correctly decrypted the first chunk and we can move on to do the same for all the other packets and reconstruct the malware by index.
...
After decompressing the file we open it in Ghidra to start the reverse engineer process. The main parts of the decompiled code are the following snippets:

> A fast way of identifying the decompiled functions related to the part of the program we want to analyze is by cross-referencing known strings and functions. For example, in this case, when we extract the program’s strings, we see `[+] Encrypted %s (size=%ld bytes)`. We can search for this string in Ghidra and immediately find where it is used, as shown in the image below.
{: .prompt-tip }

![](assets/img/Recovery/xref.png)

```c

void __cdecl FUN_004014d1(char *param_1)

{
  ...
  if (_File != (FILE *)0x0) {
    ...
    _Memory = (undefined1 *)malloc(_Size);
    if ((_DstBuf == (void *)0x0) || (_Memory == (undefined1 *)0x0)) {
      fclose(_File);
      free(_DstBuf);
      free(_Memory);
      return;
    ...
    FUN_00401460(param_1,_Memory,_Size);
    for (iVar1 = 0; iVar1 < (int)_Size; iVar1 = iVar1 + 1) {
      *(byte *)((int)_DstBuf + iVar1) = *(byte *)((int)_DstBuf + iVar1) ^ _Memory[iVar1];
    }
    rewind(_File);
    fwrite(_DstBuf,1,_Size,_File);
    fclose(_File);
    free(_DstBuf);
    free(_Memory);
...
```c
void __cdecl FUN_00401460(char *param_1,undefined1 *param_2,int param_3)

{
  ...
  for (; uVar5 != ~uVar2 - 1; uVar5 = uVar5 + 1) {
    uVar4 = uVar4 ^ (int)param_1[uVar5] << (sbyte)((uVar5 & 3) << 3);
  }
  uVar2 = 0;
  do {
    uVar5 = uVar2 + 1;
    uVar4 = uVar4 ^ (int)"evilsecretcodeforevilsecretencryption"[uVar2] << (sbyte)((uVar2 & 3) << 3)
    ;
    uVar2 = uVar5;
  } while (uVar5 != 0x25);
...
The flow of the program can be summarized as followed:

1. Opens a file, reads its entire contents into memory and allocates an equal-sized keystream buffer.
2. Builds a 32-bit seed from the filename bytes XOR-folded into a word, then XORs in a fixed literal "evilsecretcodeforevilsecretencryption".
3. Uses a 32-bit LCG (state = state * 0x19660D + 0x3C6EF35F mod 2³²) to generate N keystream bytes (low byte of state) where N = file size.
4. XORs each file byte with the corresponding keystream byte and writes the result back to the same file (in-place).
5. Prints a short confirmation message ("[+] Encrypted <filename> (size=...)").

Because XOR is symmetric, running the same routine again with the same filename/secret will restore the original file.

We develop a python script to decrypt a file from the `dump/`. 
...
MALWARE_BASE_WINDOWS_PATH = r"C:\Users\gumba\Desktop"

def generate_key_for_windows_path(win_path: str, length: int) -> bytes:
    seed = 0
    fname_bytes = win_path.encode('utf-8')
    flen = len(fname_bytes)
    slen = len(SECRET)
...
def decrypt_file(input_path: str, out_base: str = "decrypted_dump", preserve_path: bool = False) -> bool:
    if not os.path.isfile(input_path):
        print(f"[!] Error: input file not found: {input_path}")
        return False

    if preserve_path:
        rel_path = os.path.normpath(input_path).lstrip(os.sep)
        out_path = os.path.join(out_base, rel_path)
        out_dir = os.path.dirname(out_path)
    else:
        basename = os.path.basename(input_path)
        out_dir = out_base
        out_path = os.path.join(out_base, basename)

    os.makedirs(out_dir, exist_ok=True)

    basename = os.path.basename(input_path)
    windows_path = os.path.join(MALWARE_BASE_WINDOWS_PATH, basename).replace("/", "\\")

    with open(input_path, "rb") as f:
        data = f.read()

    key = generate_key_for_windows_path(windows_path, len(data))
    ...
    print(f"[+] Windows path used for keygen: {windows_path}")
    print(f"[+] Key ({len(key)} bytes): {key.hex()}")
    print(f"[+] Decrypted {len(data)} bytes")
    return True

def main():
    parser = argparse.ArgumentParser(description="Decrypt files using malware keygen (reproduces Windows path).")
    parser.add_argument("file", nargs=1, help="Path to encrypted file")
    parser.add_argument("--outdir", "-o", default="decrypted_dump", help="Output directory (default: decrypted_dump)")
    parser.add_argument("--preserve-path", action="store_true",
                        help="Preserve the input path under the output directory (creates intermediate dirs).")
    args = parser.parse_args()

    input_path = args.file[0]
    ok = decrypt_file(input_path, out_base=args.outdir, preserve_path=args.preserve_path)
    if not ok:
        sys.exit(2)
...
Running the script on the `sillyflag.png` file reveals an image with the flag on it.

```sh
shieda@pop-os:~/Desktop/CTFs/securinetsctf2025/recovery/chal$ python3 decrypt.py dump/Users/gumba/Desktop/sillyflag.png
[+] Input: dump/Users/gumba/Desktop/sillyflag.png
[+] Output: decrypted_dump/sillyflag.png
[+] Windows path used for keygen: C:\Users\gumba\Desktop\sillyflag.png
[+] Key: ...
[+] Decrypted 379686 bytes
```

![](assets/img/Recovery/sillyflag.png)


## Acknowledgements

- `@tiago3monteiro`: helped reverse engineer the malware
